...
function updatePaymentInformation(order) {
    ...
    if (order.billing.payment && order.billing.payment.selectedPaymentInstruments
        ...
        htmlToAppend += '<span>' + order.resources.cardType + ' '
            ...
            + '</span><div>'
            + order.billing.payment.selectedPaymentInstruments[1].maskedCreditCardNumber
            + '</div><div><span>'
            + order.resources.cardEnding + ' '
            + order.billing.payment.selectedPaymentInstruments[1].expirationMonth
            + '/' + order.billing.payment.selectedPaymentInstruments[1].expirationYear
            + '</span></div>';
    }
